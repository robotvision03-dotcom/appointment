# اتصال مشتری به سرویس‌دهنده (ایران)

منشی صوتی فارسی که مشتری را به فروشنده وصل می‌کند. **Twilio در ایران کار نمی‌کند.**

جریان اصلی:

1. منشی می‌پرسد: **وقت بخیر چه سرویسی را نیاز دارید؟**
2. مشتری سرویس را می‌گوید (آرایشگر، مکانیک، اورژانس، پزشک، لوله‌کش، برقکار، …).
3. فهرست سرویس‌دهندگان همان دسته نمایش داده می‌شود.
4. مشتری یکی را انتخاب می‌کند.
5. نرم‌افزار **به فروشنده زنگ می‌زند** تا مشتری برای قرار ملاقات با او صحبت کند (با کاوه‌نگار، یا لینک تماس روی صفحه).
6. اگر فروشنده جواب نداد، **شماره تماس فروشنده برای مشتری اس‌ام‌اس** می‌شود. شماره مشتری هم برای فروشنده پیامک می‌شود.

نوبت پزشکی اختیاری است و از فرم جداگانه ثبت می‌شود.

## اجرا

```bash
python3 -m venv .venv
source .venv/bin/activate          # Windows: .venv\Scripts\Activate.ps1
pip install -r requirements.txt
cp .env.example .env               # Windows: copy .env.example .env
python -m src
```

باز کنید: **http://127.0.0.1:38471**

گفتگو:

1. آرایشگاه  
2. ۳  
3. ۰۹۱۲۱۲۳۴۵۶۷  

برای هر دسته چند نمونه با شماره موبایل هست (پنج آرایشگر، پنج پزشک، پنج مکانیک، …).

## کاوه‌نگار (تماس + پیامک)

در `.env`:

```env
KAVENEGAR_API_KEY=your-key
KAVENEGAR_SENDER=1000xxxx
```

بدون کلید، تماس و پیامک فقط در لاگ ثبت می‌شوند (dry-run) و لینک تماس روی صفحه می‌ماند.

شماره‌های اضطراری مثل **۱۱۵** با کاوه‌نگار گرفته نمی‌شوند؛ لینک تماس مستقیم نشان داده می‌شود.

## ایران

| نیاز | ابزار |
| --- | --- |
| گفتگو / میکروفون | همین وب‌اپ + `/voice/live` |
| STT (شنیدن) | شنوا کوچیک v1.5 — `Reza2kn/Shenava-Koochik-v1.5` |
| تماس با فروشنده | کاوه‌نگار TTS call + پیامک |
| تماس دستی | `tel:` روی صفحه |

مدل شنیدن را یک‌بار دانلود کنید (~۱۳۰ مگابایت int8):

```powershell
pip install -r requirements.txt
py -m src download-shenava
py -m src
```

`py -m src download-whisper` و `py -m src.download_whisper` همان شنوا را دانلود می‌کنند.

اگر منشی «متوجه نشدم» می‌گوید و در لاگ `STT transcript=` خالی است: سرور را یک‌بار قطع کنید، `git pull` بگیرید، دوباره `py -m src` بزنید، و صفحه را با Ctrl+F5 تازه کنید. میکروفون باید PCM ۱۶ کیلوهرتز Int16 بفرستد؛ بدون این کار مدل شنوا متن خالی برمی‌گرداند. تایپ «آرایشگاه» همچنان کار می‌کند.

## تست

```bash
pytest tests/test_dialogue.py -q
```
